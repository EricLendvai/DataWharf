...
// Applications/ListTags/<ApplicationLinkCode>/
// Applications/NewTag/<ApplicationLinkCode>/
// Applications/EditTag/<ApplicationLinkCode>/<NameSpaceName>/
...
if len(oFcgi:p_URLPathElements) >= 2 .and. !empty(oFcgi:p_URLPathElements[2])
    ...
    case oFcgi:p_nUserAccessMode <= 1  // Application access levels
        with object l_oDB1
            ...
            :SQL(@l_aSQLResult)
            if l_oDB1:Tally == 1
                l_nAccessLevel := l_aSQLResult[1,1]
            else
                l_nAccessLevel := 0
            endif
        endwith
...
oFcgi:p_nAccessLevel := l_nAccessLevel
...
case l_cURLAction == "ApplicationLoadSchema"
    if oFcgi:p_nAccessLevel >= 6
        ...
        if oFcgi:isGet()
            ...
            if l_oDB1:Tally == 1
                l_cHtmlUnderHeader += ApplicationLoadSchemaStep1FormBuild(l_iApplicationPk,"",l_cApplicationName,l_cURLApplicationLinkCode,;
            ...
            if l_iApplicationPk > 0
                l_cHtmlUnderHeader += ApplicationLoadSchemaStep1FormOnSubmit(l_iApplicationPk,l_cApplicationName,l_cURLApplicationLinkCode)
            endif
        endif
        l_cHtml += ApplicationHeaderBuild(l_iApplicationPk,l_cApplicationName,l_cApplicationElement,l_cSitePath,l_cURLApplicationLinkCode,.t.)
    ...
    if oFcgi:isGet()
        ...
        With Object l_oDB1
            ...
            if :Tally > 0
                ...
                l_iInitialDiagramPk := val(oFcgi:GetQueryString("InitialDiagramPk"))
                if l_iInitialDiagramPk > 0
                    select ListOfDiagrams
                    locate for ListOfDiagrams->Diagram_pk == l_iInitialDiagramPk
                    if found()
                        l_iDiagramPk := l_iInitialDiagramPk
                    endif
                endif
...
case l_cURLAction == "EditTag"
    ...
    if l_oDB1:Tally != 1
        oFcgi:Redirect(oFcgi:RequestSettings["SitePath"]+"Applications/ListTags/"+l_cURLApplicationLinkCode+"/")
    else
        if oFcgi:isGet()
...
l_cHtml += [<ul class="nav nav-tabs">]
    ...
    l_cHtml += [<li class="nav-item">]
        with object l_oDB1
            ...
            l_iReccount := :Count()
        endwith
        l_cHtml += [<a class="nav-link ]+iif(par_cApplicationElement == "TAGS",[ active],[])+iif(par_lActiveHeader,[],[ disabled])+[" href="]+par_cSitePath+[Applications/ListTags/]+par_cURLApplicationLinkCode+[/">Tags (]+Trans(l_iReccount)+[)</a>]
    l_cHtml += [</li>]
    if oFcgi:p_nAccessLevel >= 7
        l_cHtml += [<li class="nav-item">]
...
l_cHtml += [<div class="m-3">]
    l_cHtml += [<table>]
        ...
        if !empty(par_iPk)
            l_cHtml += CustomFieldsBuild(par_iPk,USEDON_APPLICATION,par_iPk,par_hValues,iif(oFcgi:p_nAccessLevel >= 5,[],[disabled]))
        endif
...
case l_cActionOnSubmit == "Delete"   // Application
    if oFcgi:p_nUserAccessMode >= 3
        if CheckIfAllowDestructiveDelete(l_iApplicationPk)
            ...
            l_cHtml += [<div class="col-auto">]
                ...
                scan all
                    l_cHtml += [<tr>]
                        ...
                        l_cHtml += [<td class="GridDataControlCells" valign="top">]
                            l_cHtml += [<a href="]+l_cSitePath+[Applications/EditNameSpace/]+par_cURLApplicationLinkCode+[/]+ListOfNameSpaces->NameSpace_Name+[/">]+ListOfNameSpaces->NameSpace_Name+FormatAKAForDisplay(ListOfNameSpaces->NameSpace_AKA)+[</a>]
                        l_cHtml += [</td>]
...
l_cHtml += [<div class="m-3">]
    ...
    l_cHtml += CustomFieldsBuild(par_iApplicationPk,USEDON_NAMESPACE,par_iPk,par_hValues,iif(oFcgi:p_nAccessLevel >= 5,[],[disabled]))
...
if !empty(l_iNumberOfTablesInList)
    l_cHtml += [<div class="m-3"></div>]   //Spacer
        ...
        l_cHtml += [<div class="col-auto">]
            ...
            scan all
                ...
                l_cHtml += [<tr>]
                    ...
                    l_cHtml += [<td class="GridDataControlCells" valign="top">]
                        l_cHtml += [<a href="]+l_cSitePath+[Applications/EditTable/]+par_cURLApplicationLinkCode+[/]+Allt(ListOfTables->NameSpace_Name)+[/]+ListOfTables->Table_Name+[/">]+ListOfTables->Table_Name+FormatAKAForDisplay(ListOfTables->Table_AKA)+[</a>]
                    l_cHtml += [</td>]
    ...
    l_cHtml += [<div class="m-3">]
        ...
        if l_nNumberOfTags > 0
            l_cHtml += [<tr class="pb-5">]
                ...
                l_cHtml += [<td class="pb-3"><input]+UPDATESAVEBUTTON+[ type="text" name="TextTags" id="TextTags" value="]+FcgiPrepFieldForValue(l_cTags)+[" maxlength="200" size="80"]+iif(oFcgi:p_nAccessLevel >= 5,[],[ disabled])+[ class="form-control" placeholder=""></td>]
            l_cHtml += [</tr>]
        endif
...
case !empty(l_cErrorMessage)
    ...
    l_hValues["UseStatus"]       := l_iTableUseStatus
    l_hValues["DocStatus"]       := l_iTableDocStatus
...
if l_nNumberOfColumns <= 0
    ...
    l_cHtml += [<nav class="navbar navbar-light bg-light">]
        l_cHtml += [<div class="input-group">]
            if oFcgi:p_nAccessLevel >= 5
                l_cHtml += [<a class="btn btn-primary rounded ms-3" href="]+l_cSitePath+[Applications/NewColumn/]+par_cURLApplicationLinkCode+[/]+par_cURLNameSpaceName+[/]+par_cURLTableName+[/">New Column</a>]
            endif
            l_cHtml += [<a class="btn btn-primary rounded ms-3" href="]+l_cSitePath+[Applications/ListTables/]+par_cURLApplicationLinkCode+[/">Back To Tables</a>]
            if oFcgi:p_nAccessLevel >= 5
...
// l_cHtml += Trans(ListOfColumns->pk)
                        l_cHtml += [<a href="]+l_cSitePath+[Applications/EditColumn/]+par_cURLApplicationLinkCode+"/"+par_cURLNameSpaceName+"/"+par_cURLTableName+[/]+ListOfColumns->Column_Name+[/">]+ListOfColumns->Column_Name+FormatAKAForDisplay(ListOfColumns->Column_AKA)+[</a>]
                    l_cHtml += [</td>]
...
case l_cActionOnSubmit == "Reset"
    ...
    l_cURL := oFcgi:RequestSettings["SitePath"]+"Applications/ListColumns/"+par_cURLApplicationLinkCode+"/"+par_cURLNameSpaceName+"/"+par_cURLTableName+"/"
    oFcgi:Redirect(l_cURL)
...
l_cHtml += [<div class="m-3">]
    ...
    l_cHtml += [<tr class="pb-5">]
        ...
        l_cHtml += [<td class="pb-3">]
            ...
            l_cHtml += [<span class="pe-5">]
                l_cHtml += [<select name="ComboType" id="ComboType" onchange="OnChangeType(this.value);$('#ButtonSave').addClass('btn-warning').removeClass('btn-primary');"]+iif(oFcgi:p_nAccessLevel >= 5,[],[ disabled])+[>]  // ]+UPDATESAVEBUTTON+[
                for l_iTypeCount := 1 to len(oFcgi:p_ColumnTypes)
            ...
            l_cHtml += [<span class="pe-5" id="SpanLength" style="display: none;">]
                l_cHtml += [<span class="pe-2">Length</span><input]+UPDATESAVEBUTTON+[ type="text" name="TextLength" id="TextLength" value="]+FcgiPrepFieldForValue(l_cLength)+[" size="5" maxlength="5"]+iif(oFcgi:p_nAccessLevel >= 5,[],[ disabled])+[>]
            l_cHtml += [</span>]

            l_cHtml += [<span class="pe-5" id="SpanScale" style="display: none;">]
                l_cHtml += [<span class="pe-2">Scale</span><input]+UPDATESAVEBUTTON+[ type="text" name="TextScale" id="TextScale" value="]+FcgiPrepFieldForValue(l_cScale)+[" size="2" maxlength="2"]+iif(oFcgi:p_nAccessLevel >= 5,[],[ disabled])+[>]
            l_cHtml += [</span>]

            l_cHtml += [<span class="pe-5" id="SpanEnumeration" style="display: none;">]
                l_cHtml += [<select]+UPDATESAVEBUTTON+[ name="ComboFk_Enumeration" id="ComboFk_Enumeration"]+iif(oFcgi:p_nAccessLevel >= 5,[],[ disabled])+[>]
    ...
    l_cHtml += [<tr class="pb-5">]
        ...
        l_cHtml += [<td class="pb-3"><div class="form-check form-switch">]
            l_cHtml += [<input]+UPDATESAVEBUTTON+[ type="checkbox" name="CheckNullable" id="CheckNullable" value="1"]+iif(l_lNullable," checked","")+[ class="form-check-input"]+iif(oFcgi:p_nAccessLevel >= 5,[],[ disabled])+[>]
        l_cHtml += [</div></td>]
    l_cHtml += [</tr>]
    ...
    l_cHtml += [<tr class="pb-5" id="TRUnicode">]
        ...
        l_cHtml += [<td class="pb-3"><div class="form-check form-switch">]
            l_cHtml += [<input]+UPDATESAVEBUTTON+[ type="checkbox" name="CheckUnicode" id="CheckUnicode" value="1"]+iif(l_lUnicode," checked","")+[ class="form-check-input"]+iif(oFcgi:p_nAccessLevel >= 5,[],[ disabled])+[>]
        l_cHtml += [</div></td>]
    l_cHtml += [</tr>]
    ...
    if l_lShowPrimary
        l_cHtml += [<tr class="pb-5">]
            ...
            l_cHtml += [<td class="pb-3"><div class="form-check form-switch">]
                l_cHtml += [<input]+UPDATESAVEBUTTON+[ type="checkbox" name="CheckPrimary" id="CheckPrimary" value="1"]+iif(l_lPrimary," checked","")+[ class="form-check-input"]+iif(oFcgi:p_nAccessLevel >= 5,[],[ disabled])+[>]
            l_cHtml += [</div></td>]
        l_cHtml += [</tr>]
    endif
    ...
    l_cHtml += [<tr class="pb-5">]
        ...
        l_cHtml += [<td class="pb-3">]
            //fk_TableForeign
            l_cHtml += [<select]+UPDATESAVEBUTTON+[ name="ComboFk_TableForeign" id="ComboFk_TableForeign"]+iif(oFcgi:p_nAccessLevel >= 5,[],[ disabled])+[>]
    ...
    l_cHtml += CustomFieldsBuild(par_iApplicationPk,USEDON_COLUMN,par_iPk,par_hValues,iif(oFcgi:p_nAccessLevel >= 5,[],[disabled]))
            ...
            l_cHtml += [<div class="col-auto">]
                ...
                scan all
                    l_cHtml += [<tr>]
                        ...
                        l_cHtml += [<td class="GridDataControlCells" valign="top">]
                            l_cHtml += [<a href="]+l_cSitePath+[Applications/EditEnumeration/]+par_cURLApplicationLinkCode+[/]+ListOfEnumerations->NameSpace_Name+[/]+ListOfEnumerations->Enumeration_Name+[/">]+ListOfEnumerations->Enumeration_Name+FormatAKAForDisplay(ListOfEnumerations->Enumeration_AKA)+[</a>]
                        l_cHtml += [</td>]
    ...
    l_cHtml += [<div class="m-3">]
        ...
        l_cHtml += [<tr class="pb-5">]
            ...
            l_cHtml += [<td class="pb-3">]
                ...
                l_cHtml += [<span class="pe-5">]
                    l_cHtml += [<select]+UPDATESAVEBUTTON+[ name="ComboImplementAs" id="ComboImplementAs" onchange="OnChangeImplementAs(this.value);"]+iif(oFcgi:p_nAccessLevel >= 5,[],[ disabled])+[>]
            ...
            l_cHtml += [<span class="pe-5" id="ImplementLengthEntry" style="display: none;">]
                l_cHtml += [<span class="pe-2">Length</span><input]+UPDATESAVEBUTTON+[ type="text" size="5" maxlength="5" name="TextImplementLength" id="TextImplementLength" value="]+Trans(l_iImplementLength)+["]+iif(oFcgi:p_nAccessLevel >= 5,[],[ disabled])+[>]
            l_cHtml += [</span>]
        ...
        l_cHtml += [<div class="col-auto">]
            ...
            scan all
                l_cHtml += [<tr>]
                    ...
                    l_cHtml += [<td class="GridDataControlCells" valign="top">]
                        l_cHtml += [<a href="]+l_cSitePath+[Applications/EditEnumValue/]+par_cURLApplicationLinkCode+"/"+par_cURLNameSpaceName+"/"+par_cURLEnumerationName+[/]+ListOfEnumValues->EnumValue_Name+[/">]+ListOfEnumValues->EnumValue_Name+FormatAKAForDisplay(ListOfEnumValues->EnumValue_AKA)+[</a>]
                    l_cHtml += [</td>]
...
l_cHtml += [<div class="m-3">]
    ...
    l_cHtml += [<tr class="pb-5">]
        ...
        l_cHtml += [<td class="pb-3"><input]+UPDATESAVEBUTTON+[ type="text" name="TextNumber" id="TextNumber" value="]+FcgiPrepFieldForValue(l_cNumber)+[" maxlength="8" size="8"]+iif(oFcgi:p_nAccessLevel >= 5,[],[ disabled])+[></td>]
    l_cHtml += [</tr>]
...
if empty(l_nNumberOfTags)
    if oFcgi:p_nAccessLevel >= 5
        l_cHtml += [<nav class="navbar navbar-light bg-light">]
            l_cHtml += [<div class="input-group">]
                ...
                l_cHtml += [<a class="btn btn-primary rounded" href="]+l_cSitePath+[Applications/NewTag/]+par_cURLApplicationLinkCode+[/">New Tag</a>]
            l_cHtml += [</div>]
        l_cHtml += [</nav>]
    endif
    ...
    if oFcgi:p_nAccessLevel >= 5
        l_cHtml += [<nav class="navbar navbar-light bg-light">]
            l_cHtml += [<div class="input-group">]
                l_cHtml += [<a class="btn btn-primary rounded ms-3" href="]+l_cSitePath+[Applications/NewTag/]+par_cURLApplicationLinkCode+[/">New Tag</a>]
            l_cHtml += [</div>]
        l_cHtml += [</nav>]
            ...
            l_cHtml += [<div class="col-auto">]
                ...
                scan all
                    l_cHtml += [<tr>]
                        ...
                        l_cHtml += [<td class="GridDataControlCells" valign="top">]
                            l_cHtml += [<a href="]+l_cSitePath+[Applications/EditTag/]+par_cURLApplicationLinkCode+[/]+ListOfTags->Tag_Code+[/">]+ListOfTags->Tag_Name+[</a>]
                        l_cHtml += [</td>]
            ...
            if empty(l_cTagCode)
                ...
                if l_oDB1:Tally <> 0
                    ...
                    if l_oDB1:Tally <> 0
                        ...
                        oFcgi:Redirect(oFcgi:RequestSettings["SitePath"]+"Applications/ListTags/"+par_cURLApplicationLinkCode+"/")  //+l_cTagName+"/"
                    endif
                endif
            endif
        endif
...
case l_cActionOnSubmit == "Cancel"
    oFcgi:Redirect(oFcgi:RequestSettings["SitePath"]+"Applications/ListTags/"+par_cURLApplicationLinkCode+"/")
...
//=================================================================================================================
//=================================================================================================================
static function CascadeDeleteTable(par_iApplicationPk,par_iTablePk)
local l_cErrorMessage := ""
local l_oDB1 := hb_SQLData(oFcgi:p_o_SQLConnection)
...
//=================================================================================================================
static function CascadeDeleteNameSpace(par_iApplicationPk,par_iNameSpacePk)
...
return l_cErrorMessage
//=================================================================================================================
static function CascadeDeleteTag(par_iApplicationPk,par_iTagPk)
...
with object l_oDB1
    :Table("b32dcdfe-f196-4324-a9d0-c0a95b21850f","NameSpace")
    :Column("NameSpace.pk","pk")
    :Where("NameSpace.fk_Application = ^" , par_iApplicationPk)
    ...
    if empty(l_cErrorMessage)
        :Table("b47cd94c-420d-4405-b60e-6728a8e58277","Diagram")
        :Column("Diagram.pk","pk")
        ...
        if :Tally < 0
            ...
            select ListOfRecordsToDeleteInCascadeDeleteApplication
            scan all
                if !:Delete("e3a26e10-af99-4a22-88b9-16d18c529072","Diagram",ListOfRecordsToDeleteInCascadeDeleteApplication->pk)
            ...
            if empty(l_cErrorMessage)
                ...
                :Table("c7a11182-d771-44fc-b9cf-6be4c6b9404b","Version")
                ...
                if :Tally < 0
                    ...
                    select ListOfRecordsToDeleteInCascadeDeleteApplication
                    scan all
                        if !:Delete("5b87b713-bef5-4662-a006-e4d0dd9b5a8d","Version",ListOfRecordsToDeleteInCascadeDeleteApplication->pk)
                    ...
                    if empty(l_cErrorMessage)
                        ...
                        :Table("e1475eba-6bab-4853-89d2-6efd54a71456","ApplicationCustomField")
                        ...
                        if :Tally < 0
                            ...
                            select ListOfRecordsToDeleteInCascadeDeleteApplication
                            scan all
                                if !:Delete("1f1dfb70-418f-4b18-b553-0612e7aff79b","ApplicationCustomField",ListOfRecordsToDeleteInCascadeDeleteApplication->pk)
                            ...
                            if empty(l_cErrorMessage)
                                ...
                                if :Tally < 0
                                    ...
                                    if empty(l_cErrorMessage)
                                        ...
                                        :Table("fe5a155c-64d2-4556-a6f8-675d5533c9f8","Tag")
                                        :Column("Tag.pk","pk")
                                        :Where("Tag.fk_Application = ^" , par_iApplicationPk)
                                        :SQL("ListOfRecordsToDeleteInCascadeDeleteApplication")
                                        if :Tally < 0
                                            ...
                                            select ListOfRecordsToDeleteInCascadeDeleteApplication
                                            scan all
                                                if !:Delete("4dcf5146-91ed-4c46-8f63-a22e3ff72a35","Tag",ListOfRecordsToDeleteInCascadeDeleteApplication->pk)
                                                    l_cErrorMessage := "Failed to delete Application. Error 9."
                                                    exit
                                                endif
                                            endscan
